...
# read in the CellProfiler output from the first pilot run
now <- data.table::fread("~/repos/dauer_assay_protocol/CellProfiler/20220407_dauerProtocol1/output/20220412_run2/NonOverlappingWorms.csv")
now2 <- data.table::fread("/Users/tim/repos/dauer_assay_protocol/CellProfiler/20220407_dauerProtocol1/output/20220414_run5/NonOverlappingWorms.csv")


# Run1 and run2 do not have the relate objects module that lets us know the number of NonOverlappingWorms in each wormObject.
...
#===================================================================================#
now_proc3 <- now_proc2 %>%
  dplyr::mutate(fed = ifelse(log_cv_int2 > -2.5, T, F)) # could impirically find this line with azide treated then bead incubated worms?

# Read the rescaled RFP PNG
img <- png::readPNG(glue::glue("{getwd()}/CellProfiler/20220407_dauerProtocol1/output/20220412_run2/20220407-dauerProtocol1-p002-m2X_A01_w2_NonOverlappingWorms_RFP_mask.png"))
img2 <- png::readPNG(glue::glue("{getwd()}/CellProfiler/20220407_dauerProtocol1/output/20220412_run2/20220407-dauerProtocol1-p002-m2X_A01_w1_overlay.png"))
...
cowplot::ggsave2(a1_channels, filename = "plots/dauerProtocol1_sorter_FLchannels")
# look at the RFP for well A01 to see if it matches our expectations
ggplot(score_proc %>% dplyr::filter(col == "1" & row == "A")) +
  aes(x = TOF, y = value, fill = condition) +
  geom_point(shape = 21) +
  facet_grid(~condition + name) +
  theme_bw() +
