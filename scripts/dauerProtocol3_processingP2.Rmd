...
df_raw <- proc_dauers %>%
  ...
  dplyr::filter(condition %in% c("4hr_control", "4hr_20uMascr", "4hr_10uMascr"))
  #dplyr::filter(TOF>100)

df_green <- df_raw[c("TOF", "green", "EXT")]
ret.em_K4_green <- EMCluster::init.EM(df_green, nclass = 5, method = "em.EM") #I'm choosing 5 classes here instead of Daehan's 4 because we see 2 clusters of non-dauers in our data (Daehan see's 1)
...
ggplot(green_merge_K4) +
  ...
  labs(title = "Dauer Assay 3 20uM + 10uM + 0uM ascr, 4hr (green)") +
  theme_bw() + 
  ylim(0, 300) +
  xlim(0, 600)
```
...
green_merge_K4$class <- factor(green_merge_K4$class, levels=c(5,3,2,4,1), labels=c("dauer", "dauer2", "non-dauer", "non-dauer2", "other"))

green_merge_K4$class <- fct_collapse(green_merge_K4$class, dauer = c("dauer", "dauer2"), nondauer = c("non-dauer", "non-dauer2"), other = c("other1"))
